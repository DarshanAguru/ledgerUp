...
import { cancelScheduledNotification, schedulePushNotification } from '@/app/utils/notifications'
...
const Page = () => {
    ...
    const {getRemainder, deleteRemainder, editRemainder} = useRemaindersStore();
    const [remainder, setRemainder] = useState<any>({});

    useFocusEffect(useCallback(()=>{
      const Remainder = getRemainder(id);
      setRemainder(Remainder);
      setCategory(Remainder.repeat);
      const nd = new Date(new Date().setHours(new Date(Remainder.time).getHours(), new Date(Remainder.time).getMinutes(), 0, 0));
      setTime(nd);
    },[]))
    ...
    const onDelete = ()=>{
      ...
      cancelScheduledNotification(remainder.notifId);
      router.back();
      deleteRemainder(id);
    }
  ...
  return (
    <GestureHandlerRootView style={[styles.container]}>
        ...
        <View style={[styles.mainInputContainer]}>
            ...
            <TextInput style={[styles.input, {fontWeight: 'bold', width: '80%'}]} placeholder={remainder.title} placeholderTextColor={Colors.gray}  keyboardType='default' onChangeText={setTitle} value={title}/>
            </View>
            <View style={[styles.inputContainer, {width: '100%'}]}>
            <SelectDropdown
    data={categoriesData}
      ...
      return (
        <View style={styles.dropdownButtonStyle}>
          <Text style={[styles.dropdownButtonTxtStyle, (selectedItem && selectedItem.title)?{color: Colors.dark}:{color: Colors.gray}]}>
            {(selectedItem && selectedItem.title) || `${remainder.repeat}`}
          </Text>
  ...
  />         
        ...
        renderButton={(selectedItem, isOpened) => {
          ...
          return (
            <View style={styles.dropdownButtonStyle}>
              <Text style={[styles.dropdownButtonTxtStyle, (selectedItem && selectedItem.title)?{color: Colors.dark}:{color: Colors.gray}]}>
                {(selectedItem && selectedItem.title) || `${(remainder.day !== "")?remainder.day:"Day"}`}
              </Text>
              <Ionicons name={isOpened ? 'chevron-up' : 'chevron-down'} style={styles.dropdownButtonArrowStyle} />
            </View>
          );
      ...
      <TouchableOpacity style={[defaultStyles.pillButton, {width:'50%',backgroundColor: '#f00000',marginBottom: 20}]} onPress={onDelete}>
        <Text style={[defaultStyles.buttonText, {color: '#fff'}]}>Delete</Text>
      </TouchableOpacity>
        <TouchableOpacity style={[defaultStyles.pillButton,(category.trim() === "") ? styles.disabled:styles.enabled, {width:"50%",marginBottom: 20}]} onPress={onSave}>
